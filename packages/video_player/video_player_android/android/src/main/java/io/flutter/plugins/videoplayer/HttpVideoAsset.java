...
final class HttpVideoAsset extends VideoAsset {
    ...
    @NonNull
    private final VideoPlayerOptions videoPlayerOptions;
    ...
    HttpVideoAsset(
        ...
        this.videoPlayerOptions = options;
    }
    ...
    MediaItem getMediaItem() {
        ...
        if(this.videoPlayerOptions.useCache){
            builder.setCustomCacheKey(this.videoPlayerOptions.cacheKey);
        }
    ...
    DataSource.Factory getHlsCacheDataSourceFactory(Context context) {
        return new CacheDataSourceFactory(context,
                this.videoPlayerOptions.maxCacheSize,
                this.videoPlayerOptions.maxCacheFileSize,
                getHlsDataSourceFactory(context, new DefaultHttpDataSource.Factory()));
    }
    ...
    @VisibleForTesting
    DataSource.Factory getHlsDataSourceFactory(
            Context context, DefaultHttpDataSource.Factory initialFactory) {
        String userAgent = DEFAULT_USER_AGENT;
        if (!httpHeaders.isEmpty() && httpHeaders.containsKey(HEADER_USER_AGENT)) {
            userAgent = httpHeaders.get(HEADER_USER_AGENT);
